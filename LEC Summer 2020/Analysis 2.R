...
EGRdata <- teamstats[1:20,]

plot1 <- ggplot(EGRdata, aes(x= Statistics, y = Value, group = Team, family = "Roboto")) + 
...
#############################
save_plot("LEC Summer 2020 EGR.png", plot1, base_height = 8, base_aspect_ratio = 1.58)
############################################
EGRdata <- teamstats[1:20,]
EGRdata$Statistics <- ifelse(EGRdata$Statistics == "EGR_1", 0 , 3)
plot <- ggplot(EGRdata, aes(x= Statistics, y = Value, group = Team, family = "Roboto")) + 
  geom_line(aes(color = Team), size = 1.25) + 
  scale_color_manual(values = c('#D6D6D6','#AAC5E2','#FFB447','#FE6B64','#b27526',
                                '#D6D6D6','#D6D6D6','#769ECB','#403233','#D6D6D6')) +
  geom_dl(aes(label = Team, colour = Team), method = list("last.bumpup",cex = 1.5, dl.combine("last.points"))) + 
  geom_line(aes(y = 50), color='#72BF44', size = 1.25, linetype=2) +
  #annotate("text", x = 3, y = 49, label = "50", color = '#72BF44', size = 8) +
  annotate("rect",xmin = 0, xmax = 3, ymin = 50, ymax = 65, fill = '#BFEFBB', alpha = 0.2) +
  guides(color = FALSE) +
  labs(title = "<span style='font-size:22pt'>LEC Summer 2020: Early Game Rating</span>") +
  scale_y_continuous(limits=c(35,65), n.breaks = 6) +
...
#############################
save_plot("LEC Summer 2020 EGR 2.png", plot, base_height = 8, base_aspect_ratio = 1.58)
...
#############################
save_plot("LEC Summer 2020 MLR 2.png", plot2, base_height = 8, base_aspect_ratio = 1.58)
